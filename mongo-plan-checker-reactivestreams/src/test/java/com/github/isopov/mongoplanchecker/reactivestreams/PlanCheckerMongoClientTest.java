...
class PlanCheckerMongoClientTest extends AbstractMongoTest {
  ...
  void testCountFilter() {
    AtomicBoolean lock = new AtomicBoolean(true);
    AtomicReference<Throwable> error = new AtomicReference<>();
    collection.count(new Document("foo", "bar")).subscribe(new SingleSubscriber<>(error, lock));
  ...
  void testCountFilterGood() throws Throwable {
    ...
    collection.count(new Document("id", 333)).subscribe(new SingleSubscriber<>(error, value, lock));
